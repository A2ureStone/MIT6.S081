...
void usertrap(void)
{
  ...
  {
    printf("va addr: %p, reason: %p\n", r_stval(), r_scause());
    // p->killed = 0;
    if (lazy_alloc_mmap(r_stval(), r_scause()) != 0)
      p->killed = 1;
  }
  else
  {
...
//   if (!(access_flag & p->mmap_area[i].flags))
//     return -1;
...
//   if (readi(p->mmap_area[i].ffile->ip, 0, mem, offset, bytes) <= 0)
//   {
...
//   if (mappages(p->pagetable, addr, PGSIZE, (uint64)mem, p->mmap_area[i].flags << 1) != 0)
//   {
//     kfree(mem);
//     return -1;
//   }
