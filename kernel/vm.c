...
pagetable_t kvminit_process()
{
  ...
  if (mappages(process_kernel_pagetable, va, sz, pa, perm) != 0) 
    goto wr;
  // virtio mmio disk interface
  // kvmmapKPG(VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
  va = VIRTIO0;
  pa = VIRTIO0;
  ...
  // CLINT
  // kvmmapKPG(CLINT, CLINT, 0x10000, PTE_R | PTE_W);
  // va = CLINT;
  // pa = CLINT;
  // sz = 0x10000;
  ...
  // PLIC
  // kvmmapKPGPKBG(PLIC, PLIC, 0x400000, PTE_R | PTE_W);
  va = PLIC;
  pa = PLIC;
  sz = 0x400000;
  ...
  // map kernel text executable and read-only.
  // kvmmapKPGPKBG(KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);
  va = KERNBASE;
  pa = KERNBASE;
  ...
  // map kernel data and the physical RAM we'll make use of.
  // kvmmapKPGPKBG((uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);
  va = (uint64)etext;
  pa = (uint64)etext;
  ...
  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  // kvmmapKPGPKBG(TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
  va = TRAMPOLINE;
...
wr:
  freewalkNotLeaf(process_kernel_pagetable);
  return 0;
}
...
uvmalloc_kpg(pagetable_t pagetable, pagetable_t kpg, uint64 oldsz, uint64 newsz)
{
  ...
  for (a = oldsz; a < newsz; a += PGSIZE)
  {
    ...
    // int map2 = mappages(kpg, a, PGSIZE, (uint64)mem, PTE_W | PTE_X | PTE_R | PTE_U);
...
int uvmcopy_kpg(pagetable_t old, pagetable_t new, pagetable_t kpg, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    if ((mem = kalloc()) == 0)
      goto err;
    memmove(mem, (char *)pa, PGSIZE);
    // int map2 = mappages(kpg, i, PGSIZE, (uint64)mem, PTE_V | PTE_R | PTE_W | PTE_X);
    int map2 = mappages(kpg, i, PGSIZE, (uint64)mem, (flags & ~PTE_U));
    // int map2 = mappages(kpg, i, PGSIZE, (uint64)mem, flags);
...
uint64
walkaddr_change(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;
...
void pageTableEqual(pagetable_t upg, pagetable_t kpg) {
  for (int i = 0; i < PLIC; i += PGSIZE) {
    uint64 pte1 = walkaddr_change(upg, i);
    uint64 pte2 = walkaddr_change(kpg, i);
    if (pte1 != pte2) {
      printf("not equal va: %p\nupg pa: %p, kpg pa: %p\n", i, pte1, pte2);
    }
  }
}
...
void pgPrinter(pagetable_t pagetable) {
  for (int i = 0; i < 0x10000; i += PGSIZE) {
    pte_t* pte = walk(pagetable, i, 0);
    if (pte != 0) {
      printf("va: %p, pte: %p, pa: %p, perm: %p\n", i, *pte, PTE2PA(*pte), PTE_FLAGS(*pte));
    }
  }
}